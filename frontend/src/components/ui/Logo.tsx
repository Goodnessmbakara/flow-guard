  ...
  return (
    <Link to="/" className={`inline-flex items-center gap-2 ${className}`}>
      ...
      >
        ...
        {/* Flowing arrow loop inside hexagon - adapts to theme */}
        <path
        ...
        {/* Arrow head pointing in flow direction - forest green */}
        <path
      ...
      <div className="flex flex-col">
        <span className={`font-bold text-gray-900 dark:text-white leading-tight ${isCompact ? 'text-xl' : 'text-2xl'}`}>
          Flow
          <span className="text-gray-600 dark:text-gray-400 font-normal">{'{Treasury}'}</span>
        </span>
        {showTagline && (
          <span className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">On-chain treasury management</span>
        )}
      </div>
    </Link>
  );
...
export const LogoIcon: React.FC<{ size?: number; className?: string }> = ({
  ...
  return (
    ...
    >
      ...
      {/* Flowing arrow loop inside hexagon - forest green */}
      <path
        d="M20 14 C 16.5 14, 14 16.5, 14 20 C 14 23.5, 16.5 26, 20 26 C 23.5 26, 26 23.5, 26 20 C 26 16.5, 23.5 14, 20 14"
        stroke="#4b6e48"
        strokeWidth="2.5"
...
export const LogoText: React.FC<{ className?: string }> = ({ className = '' }) => {
  return (
    <Link to="/" className={`font-bold text-gray-900 dark:text-white ${className}`}>
      Flow
      <span className="text-gray-600 dark:text-gray-400 font-normal">{'{Treasury}'}</span>
    </Link>
  );
