...
Here are the core feature sets, with functional requirements, and how they map to Layla CHIPs.
...
Mitigation
Smart Contract Bugs
Thorough testing + peer review + use of simple covenant patterns. Limit scope in MVP.
Cost Overrun (On-Chain Fees)
Optimize covenant size; minimize number of script actions; batch proposals.
Lack of Adoption
Target pilot BCH teams / devs; partner with BCH ecosystem DAOs; integrate with BLISS / hackathon networks.
Signer Collusion or Malicious Signers
Use multi-signature threshold >1; allow signer removal via governance; require clear identity or reputation for signers.
Backend Dependency
Design so key flows (propose, approve, payout) can be done entirely on-chain via wallet. Backend only optional.
User UX Complexity
Build simple UI; provide clear guidance (wizard) for vault creation; minimize jargon (“Loop,” “covenant,” etc.).
...
Intro (30s): “FlowGuard is on-chain treasury guardrails for BCH — safe, automated, permissioned.”


Vault Creation (1 min): Show wizard → define unlock schedule, signers, threshold → fund with BCH in wallet → submit deposit.


Loop Activation (1 min): Show loop cycle logic — backend / wallet detects the first cycle window, loop triggers “unlockable funds.”


Spending Proposal (1 min): Create a proposal (recipient, amount, reason) → show it waiting for approvals.


Approvals (45s): Signer 1 approves; signer 2 approves via wallet → both on-chain.


Payout Execution (45s): Covenant executes P2S transaction → funds released; bitwise state updates.


State Dashboard (30s): Show bitwise state, cycles status, history log, and transparency.
